...
# ---------------------------------------------------------------------------
# C. Duplicate replay via direct Lambda invoke (fixed sourceEventId)
# ---------------------------------------------------------------------------
if [[ "${SKIP_IDEMPOTENCY_REPLAY}" != "true" ]]; then
  echo "==> [replaying duplicate] direct processor invoke ×2"
  REPLAY_EVENT_ID="scrum42-replay-${COMMIT_SHA//[^a-zA-Z0-9]/}"
  node --input-type=module - "${TMP_DIR}/replay-envelope.json" "${REPLAY_EVENT_ID}" <<'NODE'
...
NODE
  ...
  aws lambda invoke \
    ...
    --payload "file://${TMP_DIR}/replay-envelope.json" \
    "${FIRST_OUT}" >/dev/null
  aws lambda invoke \
    --function-name "${PROCESSOR_FUNCTION_NAME}" \
    --cli-binary-format raw-in-base64-out \
  ...
  python3 - "${FIRST_OUT}" "${SECOND_OUT}" "${OUT_DIR}/duplicate-replay.sanitized.json" <<'PY'
import json, sys
first = json.load(open(sys.argv[1]))
second = json.load(open(sys.argv[2]))
out = sys.argv[3]
# Lambda may wrap payload
def unwrap(doc):
...
safe = {
    "first": {
        "persistedIncidents": first.get("persistedIncidents"),
        "duplicateIncidents": first.get("duplicateIncidents"),
        "analysisAttempts": first.get("analysisAttempts"),
        "analyzedIncidents": first.get("analyzedIncidents"),
        "notificationAttempts": first.get("notificationAttempts"),
        "notificationsSent": first.get("notificationsSent"),
    },
    "second": {
        "persistedIncidents": second.get("persistedIncidents"),
        "duplicateIncidents": second.get("duplicateIncidents"),
        "analysisAttempts": second.get("analysisAttempts"),
        "analyzedIncidents": second.get("analyzedIncidents"),
        "notificationAttempts": second.get("notificationAttempts"),
        "notificationsSent": second.get("notificationsSent"),
    },
}
json.dump(safe, open(out, "w"), indent=2)
ok = (
    safe["first"].get("persistedIncidents") == 1
    and safe["first"].get("analysisAttempts") == 1
    and safe["first"].get("analyzedIncidents") == 1
    and safe["second"].get("duplicateIncidents") == 1
    and safe["second"].get("analysisAttempts") == 0
    and (safe["second"].get("notificationAttempts") or 0) == 0
)
...
PY
  ...
  pass_idem="pass"
  echo "    duplicate replay OK (first create/analyze/notify; second duplicate only)"
